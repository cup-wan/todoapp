...
@Log
@RequestMapping("/schedules") //base url 설정, url 컨트롤러의 메서드와 매핑할 때 사용
public class TodoController {
    ...
    public ResponseEntity<?> create(@RequestBody ScheduleCreateReq request){
    ...
    public ResponseEntity<ScheduleReadRes> readOne(@PathVariable Long id){
        log.info("READ ONE");

        Todo result = this.todoService.searchById(id);
        return ResponseEntity.ok(new ScheduleReadRes(result));
    }

    @GetMapping //모든 todoitem을 조회하는 api
